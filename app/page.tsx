...
type InstagramStats = { followers: number | null; reach: number | null; views: number | null; posts: number | null };
const formatStat = (value: number | null) => value === null ? "—" : new Intl.NumberFormat("en", { notation: "compact", maximumFractionDigits: 1 }).format(value);
export default function Home() {
  const [started, setStarted] = useState(false); const [activeSkill, setActiveSkill] = useState("Select a node to inspect capability data."); const [showEmail, setShowEmail] = useState(false); const [instagramStats, setInstagramStats] = useState<InstagramStats | null>(null);
  ...
  useEffect(() => { fetch("/api/instagram").then(response => response.ok ? response.json() : null).then((data: InstagramStats | null) => setInstagramStats(data)).catch(() => setInstagramStats(null)); }, []);
  return <main id="top" className={started ? "started" : ""}>
    ...
    <div className="content">
      ...
      <Section id="creator" kicker="04 / CREATOR ARENA" title="SIGNAL & INFLUENCE">
        <motion.div className="creator-grid" {...fade}><div className="social-card"><div className="social-top"><span>◉ INSTAGRAM</span><b>{instagramStats ? "LIVE SIGNAL" : "CREATOR SPACE"}</b></div><div className="metric"><strong>YOUR</strong><span>CONTENT<br />UNIVERSE</span></div><p>Content creation is my creative hobby—a separate space to share interests, ideas, and moments beyond my engineering work.</p><a href="https://www.instagram.com/o5_pratik" target="_blank" rel="noreferrer">OPEN CHANNEL ↗</a></div><div className="dashboard">{[["FOLLOWERS", instagramStats?.followers ?? null], ["REACH", instagramStats?.reach ?? null], ["VIEWS", instagramStats?.views ?? null], ["POSTS", instagramStats?.posts ?? null]].map(([label, value]) => <div className="dash-card" key={label as string}><small>{label}</small><strong>{formatStat(value as number | null)}</strong><span>{instagramStats ? "LIVE FROM INSTAGRAM" : "CONNECTING"} <i /></span></div>)}<div className="categories"><small>CONTENT CATEGORIES</small><p>LIFESTYLE <i /> CREATIVITY <i /> ENTERTAINMENT <i /> PERSONAL JOURNEY</p></div></div></motion.div>
      </Section>
